...
* 支持实盘行情订阅和数据存储（tick,bar），类似vnpy的data recorder, 单独进程模式，同时也具有vnpy的csv loader的功能，支持bar和tick数据导入；支持从多个数据源下载数据（RQData，Tushare，JoinQuant）；支持多种回测数据来源；支持bar和tick不同时间尺度回测；
* 支持实盘行情数据的模拟交易（简单的撮合）；
* 采用Qt可视化界面作为前端，方便管理，监控和操作，相关监控信息可以导出为csv文件，可以查看实时k线数据；可以查看合约基本信息；可以选择性显示/隐藏指定视图控件单元；
