...
/* 默认开启用于统计延时的打点信息 */
        ...
        && ! defined (_MDS_ENABLE_LATENCY_STATS) && 1
...
/** Level2增量更新的价位列表最大数量 */
#define MDS_MAX_L2_PRICE_LEVEL_INCREMENTS       (24)

/** Level2披露的买一／卖一委托明细最大数量 */
#define MDS_MAX_L2_DISCLOSE_ORDERS_CNT          (50)

/** Level2增量更新的委托明细最大数量 */
#define MDS_MAX_L2_DISCLOSE_ORDERS_INCREMENTS   (104)
/* -------------------------           */
...
typedef enum _eMdsMsgSource {
    ...
    MDS_MSGSRC_MDGW_BINARY          = 5,        /**< 消息来源-SZSE-MDGW-Binary(TCP) */
    MDS_MSGSRC_MDGW_STEP            = 6,        /**< 消息来源-SZSE-MDGW-Binary(TCP) */
...
typedef enum _eMdsMdStreamType {
    ...
    MDS_MD_STREAM_TYPE_STOCK            = 2,    /**< 上证所股票 */
    MDS_MD_STREAM_TYPE_BOND             = 3,    /**< 上证所债券(Level1) */
    MDS_MD_STREAM_TYPE_FUND             = 4,    /**< 上证所基金(Level1) */
...
typedef enum _eMdsMdLevel {
    MDS_MD_LEVEL_1                  = 0,        /**< Level-1 行情 */
    MDS_MD_LEVEL_2                  = 1,        /**< Level-2 行情 */
    __MAX_MDS_MD_LEVEL
} eMdsMdLevelT;
...
typedef struct _MdsPriceLevelEntry {
    int32               Price;                  /**< 委托价格 */
    int32               NumberOfOrders;         /**< 实际的总委托笔数 (Level1不揭示该值, 固定为0) */
    int64               OrderQty;               /**< 委托数量 */
} MdsPriceLevelEntryT;
...
typedef struct _MdsMktDataSnapshotHead {
    ...
    int32               updateTime;             /**< 行情时间 (HHMMSSsss, 交易所时间, 9位整型数值) */
 ...
 * 股票(A、B股)、债券、基金
 *
 * 集合竞价期间的虚拟成交价通过买卖盘档位揭示:
 * - 其中买一和卖一都揭示虚拟成交价格和成交数量
...
typedef struct _MdsL2StockSnapshotBody {
    ...
    int32               BidPriceLevel;          /**< 买方委托价位数 */
    int32               OfferPriceLevel;        /**< 卖方委托价位数 */
...
/**
 * Level2 委托队列的增量更新信息（买一／卖一前五十笔委托明细）
 */
