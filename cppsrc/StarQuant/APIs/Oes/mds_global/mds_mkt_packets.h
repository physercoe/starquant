...
/** 当前采用的协议版本号 */
#define MDS_APPL_VER_ID                         "0.15.5.1"
/**
 * 当前采用的协议版本号数值
 ...
 */
#define MDS_APPL_VER_VALUE                      (1001505011)
...
typedef enum _eMdsMsgType {
    ...
    /** Level2 逐笔数据丢失消息 (28/0x1C) (TODO @depricated 已废弃) */
    MDS_MSGTYPE_L2_TICK_LOST                        = 28,
...
typedef enum _eMdsSubscribeDataType {
    /** 所有数据 */
    MDS_SUB_DATA_TYPE_ALL                       = 0,

    /** L1快照/指数/期权 */
    MDS_SUB_DATA_TYPE_L1_SNAPSHOT               = 0x01,

    /** L2快照 */
    MDS_SUB_DATA_TYPE_L2_SNAPSHOT               = 0x02,

    /** L2委托队列 */
    MDS_SUB_DATA_TYPE_L2_BEST_ORDERS            = 0x04,

    /** L2逐笔成交 */
    MDS_SUB_DATA_TYPE_L2_TRADE                  = 0x08,

    /** L2逐笔委托（*深圳, 0x10/16） */
    MDS_SUB_DATA_TYPE_L2_ORDER                  = 0x10,

    /** L2虚拟集合竞价（*上海, 0x20/32） */
    MDS_SUB_DATA_TYPE_L2_VIRTUAL_AUCTION_PRICE  = 0x20,

    /** L2市场总览（*上海, 0x40/64） */
    MDS_SUB_DATA_TYPE_L2_MARKET_OVERVIEW        = 0x40,

    /** 市场状态（*上海, 0x100/256） */
    MDS_SUB_DATA_TYPE_TRADING_SESSION_STATUS    = 0x100,

    /** 证券实时状态（*深圳, 0x200/512） */
    MDS_SUB_DATA_TYPE_SECURITY_STATUS           = 0x200,

    /** 空数据种类 (可用于不指定任何数量种类的情况) */
    MDS_SUB_DATA_TYPE_NONE                      = 0x8000,

    __MAX_MDS_SUB_DATA_TYPE                     = 0x10000
...
typedef struct _MdsMktDataRequestEntry {
    uint8               exchId;                 /**< 交易所代码(沪/深) */
    uint8               securityType;           /**< 证券类型，01 股票(含指数) 02 衍生品 */
...
typedef struct _MdsMktDataRequestReq {
     ...
     *       - 该模式推送的数据量最小, 没有重复数据，也不会重复发送最新快照
     *       - 该模式在时延和带宽方面都相对优秀，如果没有特殊需求，推荐使用该模式即可
     * -  1: (AllIncrements) 订阅所有时点的行情快照数据 (包括Level2增量更新消息)
     *       - 该模式会推送所有时点的行情数据，包括Level2行情快照的增量更新消息
     *       - 如果需要获取全量的行情明细，或者需要直接使用Level2的增量更新消息，可以使用该模式
     * -  2: (LatestTimely) 只订阅最新的行情快照数据, 并立即发送最新数据
     *       - 只要有行情更新事件，便立即推送该产品的最新行情，但也会因此重复发送多次相同的最新行情
     *       - 如果某些产品的交易很活跃，而客户端处理又比较耗时的话，那么该模式可能会更及时的获取到
     *         这些产品的最新行情
     *       - 此外，因为与 AllIncrements 模式下的数据一一对应，可以方便与增量更新消息进行比对测试
