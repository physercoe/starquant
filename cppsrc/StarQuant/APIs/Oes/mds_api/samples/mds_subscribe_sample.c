...
MdsApiSample_HandleMsg(MdsApiSessionInfoT *pSessionInfo,
    ...
    case MDS_MSGTYPE_L2_TRADE:
        /* 处理Level2逐笔成交消息 */
        printf("... 接收到Level2逐笔成交消息 (exchId[%hhu], instrId[%d])\n",
    ...
    case MDS_MSGTYPE_L2_ORDER:
        /* 处理Level2逐笔委托消息 */
        printf("... 接收到Level2逐笔委托消息 (exchId[%hhu], instrId[%d])\n",
    ...
    case MDS_MSGTYPE_INDEX_SNAPSHOT_FULL_REFRESH:
        /* 处理Level1快照行情消息 */
        printf("... 接收到Level1快照行情消息 (exchId[%hhu], instrId[%d])\n",
                pRspMsg->mktDataSnapshot.head.exchId,
                pRspMsg->mktDataSnapshot.head.instrId);
        break;
    ...
    case MDS_MSGTYPE_SECURITY_STATUS:
        /* 处理(深圳)证券状态消息 */
        printf("... 接收到(深圳)证券状态消息 (exchId[%hhu], instrId[%d])\n",
    ...
    case MDS_MSGTYPE_TRADING_SESSION_STATUS:
        /* 处理(上证)市场状态消息 */
        printf("... 接收到(上证)市场状态消息 (exchId[%hhu], TradingSessionID[%s])\n",
    ...
    default:
        fprintf(stderr, "无效的消息类型! msgId[0x%02X], server[%s:%d]",
