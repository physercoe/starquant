...
OesClientMySpi::OnBusinessReject(int32 errorCode, const OesOrdRejectT *pOrderReject) {
    fprintf(stdout, ">>> 收到委托业务拒绝回报: " \
            "客户端环境号[%hhd], 客户委托流水号[%d], 证券账户[%s], 证券代码[%s], " \
            "市场代码[%hhu], 委托类型[%hhu], 买卖类型[%hhu], 委托数量[%d], " \
            "委托价格[%d], 原始委托的客户订单编号[%lld], 错误码[%d]\n",
...
OesClientMySpi::OnOrderInsert(const OesOrdCnfmT *pOrderInsert) {
    fprintf(stdout, ">>> 收到委托已收回报: " \
            "客户端环境号[%hhd], 客户委托流水号[%d], 会员内部编号[%lld], " \
            "证券账户[%s], 证券代码[%s], 市场代码[%hhu], 订单类型[%hhu], " \
            "买卖类型[%hhu], 委托状态[%hhu], 委托日期[%d], 委托接收时间[%d], " \
            "委托确认时间[%d], 委托数量[%d], 委托价格[%d], 撤单数量[%d], " \
            "累计成交份数[%d], 累计成交金额[%lld], 累计债券利息[%lld], " \
            "累计交易佣金[%lld], 冻结交易金额[%lld], 冻结债券利息[%lld], " \
            "冻结交易佣金[%lld], 被撤内部委托编号[%lld], 拒绝原因[%d], " \
            "交易所错误码[%d]\n",
...
OesClientMySpi::OnTradeReport(const OesTrdCnfmT *pTradeReport) {
    fprintf(stdout, ">>> 收到成交回报: " \
            "成交编号[%lld], 会员内部编号[%lld], 委托客户端环境号[%hhd], " \
            "客户委托流水号[%d], 证券账户[%s], 证券代码[%s], 市场代码[%hhu], " \
            "买卖方向[%hhu], 委托买卖类型[%hhu],  成交日期[%d], 成交时间[%d], " \
            "成交数量[%d], 成交价格[%d], 成交金额[%lld], 累计成交数量[%d], " \
            "累计成交金额[%lld], 累计债券利息[%lld], 累计交易费用[%lld], PBU代码[%d]\n",
...
OesClientMySpi::OnCashAssetVariation(const OesCashAssetItemT *pCashAssetItem) {
    fprintf(stdout, ">>> 收到资金变动回报: " \
            "资金账户代码[%s], 客户代码[%s], 币种[%hhu], 资金类型[%hhu], " \
            "资金账户状态[%hhu], 期初余额[%lld], 期初可用余额[%lld], " \
            "期初可取余额[%lld], 不可用余额[%lld],累计存入金额[%lld], " \
            "累计提取金额[%lld], 当前提取冻结金额[%lld], 累计卖金额[%lld], " \
            "累计买金额[%lld], 当前买冻结金额[%lld], 累计费用金额[%lld], " \
            "当前费用冻结金额[%lld], 当前维持保证金金额[%lld], " \
            "当前保证金冻结金额[%lld], 当前余额[%lld], 当前可用余额[%lld], " \
            "当前可取余额[%lld]\n",
...
OesClientMySpi::OnStockHoldingVariation(const OesStkHoldingItemT *pStkHoldingItem) {
    fprintf(stdout, ">>> 收到持仓变动回报: " \
            "证券账户[%s], 市场代码[%hhu], 产品代码[%s], 日初持仓[%lld], " \
            "日中累计买入持仓[%lld], 日中累计卖出持仓[%lld], 当前卖出冻结持仓[%lld], " \
            "手动冻结持仓[%lld], 日中累计转换获得持仓[%lld], 日中累计转换付出持仓[%lld], " \
            "当前转换付出冻结持仓[%lld], 当前已锁定持仓[%lld], 当前锁定冻结持仓[%lld], " \
            "当前解锁定冻结持仓[%lld], 当前备兑冻结的现货持仓[%lld], " \
            "当前已备兑使用的现货持仓[%lld], 日初总持仓成本[%lld], 当日累计买入金额[%lld], " \
            "当日累计卖出金额[%lld], 当日累计买入费用[%lld], 当日累计卖出费用[%lld], " \
            "持仓成本价[%lld], 当前总持仓[%lld], 当前可卖持仓[%lld], " \
            "当前可转换付出持仓[%lld], 当前可锁定持仓[%lld], 当前可备兑/解锁持仓[%lld]\n",
            pStkHoldingItem->invAcctId, pStkHoldingItem->mktId,
            pStkHoldingItem->securityId, pStkHoldingItem->originalHld,
...
OesClientMySpi::OnFundTrsfReject(int32 errorCode,
    ...
    fprintf(stdout, ">>> 收到出入金委托拒绝回报: " \
            "错误码[%d], 错误信息[%s], 客户端环境号[%hhd], 出入金流水号[%d], " \
            "资金账户[%s], 是否仅调拨[%hhu], 出入金方向[%hhu], 出入金金额[%lld]\n",
...
OesClientMySpi::OnFundTrsfReport(int32 errorCode,
    ...
    fprintf(stdout, ">>> 收到出入金委托执行回报: " \
            "错误原因[%d], 主柜错误码[%d], 错误信息[%s], 客户端环境号[%hhd], "
            "出入金流水号[%d], 出入金编号[%d], 资金账户[%s], 是否仅调拨[%hhu], "
            "出入金方向[%hhu], 出入金金额[%lld], 出入金状态[%d], 接收日期[%08d], "
...
OesClientMySpi::OnQueryCashAsset(const OesCashAssetItemT *pCashAsset,
    ...
    fprintf(stdout, ">>> 查询到资金信息: index[%d], isEnd[%c], " \
            "资金账户代码[%s], 客户代码[%s], 币种[%hhu], 资金类型[%hhu], " \
            "期初余额[%lld], 期初可用[%lld], 期初可取[%lld], 不可用[%lld], " \
            "累计存入[%lld], 累计提取[%lld], 当前提取冻结[%lld], " \
            "累计卖出[%lld], 累计买入[%lld], 当前买冻结[%lld], " \
            "累计费用[%lld], 当前费用冻结[%lld], 当前维持保证金[%lld], "
            "当前保证金冻结[%lld], 当前余额[%lld], 当前可用[%lld], 当前可取[%lld]\n",
            pCursor->seqNo, pCursor->isEnd ? 'Y' : 'N',
...
OesClientMySpi::OnQueryStkHolding(const OesStkHoldingItemT *pStkHolding,
    ...
    fprintf(stdout, ">>> 查询到股票持仓信息: index[%d], isEnd[%c], " \
            "证券账户[%s], 市场代码[%hhu], 产品代码[%s], 日初持仓[%lld], " \
            "日中累计买入[%lld], 日中累计卖出[%lld], 当前卖出冻结[%lld], " \
            "日中累计转换获得[%lld], 日中累计转换付出[%lld], 当前转换付出冻结[%lld], " \
            "当前已锁定[%lld], 当前锁定冻结[%lld], 当前解锁定冻结[%lld], " \
            "当前备兑冻结[%lld], 当前已备兑使用[%lld], 当前可备兑/解锁[%lld], " \
            "当前总持仓[%lld], 当前可卖[%lld], 当前可转换付出[%lld], 当前可锁定[%lld]\n",
            pCursor->seqNo, pCursor->isEnd ? 'Y' : 'N',
...
OesClientMySpi::OnQueryLotWinning(const OesLotWinningItemT *pLotWinning,
    ...
    fprintf(stdout, ">>> 查询到新股配号、中签信息: index[%d], isEnd[%c], " \
            "股东账户代码[%s], 证券代码[%s], 证券名称[%s], 市场代码[%hhu], " \
            "记录类型[%hhu], 失败原因[%hhu], 配号、中签日期[%08d], " \
            "配号首个号码[%lld], 配号成功数量/中签股数[%d], " \
            "最终发行价[%d], 中签金额[%lld]\n",
            pCursor->seqNo, pCursor->isEnd ? 'Y' : 'N',
...
OesClientMySpi::OnQueryCustInfo(const OesCustItemT *pCust,
    ...
    fprintf(stdout, ">>> 查询到客户信息: index[%d], isEnd[%c], " \
            "客户ID[%s], 客户类型[%hhu], 客户状态[%hhu], 风险评级[%hhu], "
            "机构标志[%hhu], 投资者分类[%c]\n",
            pCursor->seqNo, pCursor->isEnd ? 'Y' : 'N',
...
OesClientMySpi::OnQueryInvAcct(const OesInvAcctItemT *pInvAcct,
    ...
    fprintf(stdout, ">>> 查询到证券账户信息: index[%d], isEnd[%c], " \
            "证券账户[%s], 市场代码[%hhu], 客户代码[%s], 账户状态[%hhu], "
            "新股认购限额[%d]\n",
            pCursor->seqNo, pCursor->isEnd ? 'Y' : 'N',
...
OesClientMySpi::OnQueryCommissionRate(
    ...
    fprintf(stdout, ">>> 查询到佣金信息: index[%d], isEnd[%c], " \
            "客户代码[%s], 证券代码[%s], 市场代码[%hhu], 证券类型[%hhu], " \
            "证券子类型[%hhu], 买卖类型[%hhu], 币种[%hhu], 费用标识[%hhu], " \
            "计算模式 [%hhu], 费率[%lld], 最低费用[%d], 最高费用[%d]\n",
            pCursor->seqNo, pCursor->isEnd ? 'Y' : 'N',
...
OesClientMySpi::OnQueryFundTransferSerial(
    ...
    fprintf(stdout, ">>> 查询到出入金流水: index[%d], isEnd[%c], " \
            "客户端环境号[%hhd], 客户委托流水号[%d], " \
            "资金账户[%s], 方向[%s], 金额[%lld], 出入金状态[%hhu], " \
            "错误原因[%d], 主柜错误码[%d], 错误信息[%s], 柜台委托编码[%d], 接收日期[%08d], " \
            "接收时间[%09d], 上报时间[%09d], 完成时间[%09d]\n",
            pCursor->seqNo, pCursor->isEnd ? 'Y' : 'N',
...
OesClientMySpi::OnQueryIssue(const OesIssueItemT *pIssue,
    ...
    fprintf(stdout, ">>> 查询到证券发行产品信息: index[%d], isEnd[%c], " \
            "证券代码[%s], 证券名称[%s], 正股代码[%s], 市场代码[%hhu], " \
            "证券类型[%hhu], 证券子类型[%hhu], 是否允许撤单[%hhu], " \
            "是否允许重复认购[%hhu], 发行起始时间[%d], 发行结束时间[%d], " \
            "发行总量[%lld], 份数单位[%d], 最大份数[%d], " \
            "最小份数[%d], 发行价格[%d], 价格上限[%d], 价格下限[%d]\n",
            pCursor->seqNo, pCursor->isEnd ? 'Y' : 'N',
...
OesClientMySpi::OnQueryStock(const OesStockItemT *pStock,
    ...
    fprintf(stdout, ">>> 查询到现货产品信息: index[%d], isEnd[%c], " \
            ...
            "市场代码[%hhu], 证券类型[%hhu], 证券子类型[%hhu], " \
            "证券级别[%hhu], 风险等级[%hhu], 停牌标志[%hhu], 适当性管理[%hhu], " \
            "当日回转[%hhu], 价格单位[%d], 买份数单位[%d], 卖份数单位[%d], " \
            "昨日收盘价[%d], 债券利息[%lld], 涨停价[%d], 跌停价[%d]\n",
            pCursor->seqNo, pCursor->isEnd ? 'Y' : 'N',
...
OesClientMySpi::OnQueryEtf(const OesEtfItemT *pEtf,
    ...
    fprintf(stdout, ">>> 查询到ETF申赎产品信息: index[%d], isEnd[%c], " \
            "基金代码[%s], 证券代码[%s], 市场代码[%hhu], 申购赎回单位[%d], " \
            "最大现金替代比例[%d], 前一日最小申赎单位净值[%lld], 前一日现金差额[%lld], " \
            "成分证券数目[%d]\n",
            pCursor->seqNo, pCursor->isEnd ? 'Y' : 'N',
...
OesClientMySpi::OnQueryEtfComponent(const OesEtfComponentItemT *pEtfComponent,
    ...
    fprintf(stdout, ">>> 查询到ETF成分股信息: index[%d], isEnd[%c], " \
            "成分证券代码[%s], 市场代码[%hhu], 现金替代标识[%d], " \
            "昨日收盘价[%d], 成分证券数量[%d], 溢价比例[%d], " \
            "申购替代金额[%lld], 赎回替代金额[%lld]\n", \
            pCursor->seqNo, pCursor->isEnd ? 'Y' : 'N',
