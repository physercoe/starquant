...
from pystarquant.common.datastruct import TickData, BarData
...
class BarGenerator:
    ...
    def update_tick(self, tick: TickData):
        ...
        # Filter tick data with no last price
        if not tick.last_price:
            return
        if not tick.ask_price_1 and not tick.bid_price_1:
            return
        ...
        # in backtest when switching day there should be a new bar
            ...
            # filter useless tick in live data, usally before open and after close time
        ...
        if new_minute:
            #TODO: consider whether use last tick's price as newbar's openprice, thus more continuous and jump will show
            
            self.bar = BarData(
                full_symbol=tick.full_symbol,
                symbol=tick.symbol,
    ...
    def update_bar(self, bar: BarData):
        ...
        if self.interval == Interval.MINUTE:
            ...
            if not (bar.datetime.minute + 1) % self.window:
                finished = True
        # TODO: this method delay one bar
            ...
            if self.last_bar and bar.datetime.hour != self.last_bar.datetime.hour:
                ...
                # x-hour bar
                    ...
                    if not self.interval_count % self.window:
                        finished = True
                        self.interval_count = 0
        # TODO: simple method, only used in tdx data us data, in which the date is the local tzone
...
class VolumeBarGenerator:
    """
    For: 
    1. generating 5000 volume bar data from tick data
    2  generating x* 5000 volume bar data from tick data
    ...
    def update_tick(self, tick: TickData):
        ...
        if not self.bar:
            new_volume = True
        # in backtest when switching day there should be a new bar
        elif self.bar.volume > self.interval or (tick.datetime - self.bar.datetime) > timedelta(seconds=3600):
            # filter useless tick in live data, usally before open and after close time
            # if self.tick_counts > 5:
            #     self.on_bar(self.bar)
            self.on_bar(self.bar)
    ...
    def update_bar(self, bar: BarData):
        ...
        if self.window_bar.volume > self.interval * self.window or (bar.datetime - self.window_bar.datetime) > timedelta(seconds=14400):
            finished = True
