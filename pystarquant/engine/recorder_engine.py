...
from pystarquant.common.utility import load_json, save_json
...
# event handler
    ...
    def process_tick_event(self, event: Event):
        ...
        if dayclosetime or nightclosetime:
            return
        if abs(tick.datetime - datetime.now()) > timedelta(seconds=600):
            return
    ...
    def record_tick(self, tick: TickData):
        """"""
        # 忽略与当时时间差很多的tick
        if abs(tick.datetime - datetime.now()) > timedelta(seconds=3600):
            return 
        
        try:
            database_manager.save_tick_data([tick],True)
        except:
            print('record tick error:',tick.full_symbol,tick.datetime)

    def record_bar(self, bar: BarData):
        """"""
        try:
            database_manager.save_bar_data([bar],True)
        except:
            print('record bar error:',bar.full_symbol,bar.datetime)
