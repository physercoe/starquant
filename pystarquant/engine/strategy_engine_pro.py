...
from nanomsg import Socket, SUB, PUSH, SUB_SUBSCRIBE, SOL_SOCKET, RCVTIMEO, RCVBUF
...
from pystarquant.common.utility import extract_full_symbol, load_json, save_json
...
from pystarquant.engine.iengine import BaseEngine, EventEngine

CtaTemplate = StrategyBase


class StrategyEnginePro(BaseEngine):
    """
    Send to and receive from msg  server ,used for strategy 
    """
    config_filename = "config_server.yaml"
    setting_filename = "cta_strategypro_setting.json"
    data_filename = "cta_strategypro_data.json"
# init
...
# init functions
    ...
    def init_nng(self):
        ...
        # self._recv_sock.set_int_option(SOL_SOCKET, RCVTIMEO, 10000)
        self._recv_sock.set_int_option(SOL_SOCKET,RCVBUF, 1024*1024*128)
        ...
        for dirpath, dirnames, filenames in os.walk(path):
            for filename in filenames:
                if filename.endswith(".py"):
                    ...
                    strategy_module_name = "mystrategy.".join(
                        [module_name, filename.replace(".so", "")])       
