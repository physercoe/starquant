...
from pystarquant.common.constant import (EventType, MSG_TYPE, Exchange, Interval,
...
from pystarquant.common.utility import generate_full_symbol, extract_full_symbol, generate_vt_symbol
...
class TBTBarData(BaseData):
    ...
    # new field for stock 
    ask_totalqty: float = 0   # 总委托卖量
    bid_totalqty: float = 0   # 总委托买量
    ask_totalmoney: float = 0  # 总委托卖额
    bid_totalmoney: float = 0  # 总委托买额
    ask_bigmoney: float = 0   # 大单流出总额（累加）
    bid_bigmoney: float = 0   # 大单流入总额（累加）
    ask_bigleft: float = 0  # 存量大单卖额
    bid_bigleft: float = 0  # 存量大单买额

    #user defined
    user_defined_1: float = 0.0 #自定义变量1
...
class OrderData(BaseData):
    ...
    sec_type: Product = Product.EQUITY
...
class ContractData(BaseData):
    ...
    slippage: float = 0.0 
    rate: float = 0.0     #手续费
    isratio:bool = True    #按金额还是手数
    close_rate: float = 0.0  #平今费用
