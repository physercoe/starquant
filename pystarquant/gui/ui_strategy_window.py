...
from pystarquant.common.constant import EventType, MSG_TYPE
...
class CtaManager(QtWidgets.QWidget):
    ...
    signal_strategy_internal = QtCore.pyqtSignal(Event)
    ...
    def watch_strategy(self, strategydata: dict,wid:str):

        m = Event(type=EventType.STRATEGY_CONTROL, data=strategydata,
                  des='@w' + wid, src='0', msgtype=MSG_TYPE.MSG_TYPE_STRATEGY_WATCH)
        self.signal_strategy_internal.emit(m)        
...
class StrategyManager(QtWidgets.QFrame):
    ...
    def init_ui(self):
        ...
        watchlistcombo = QtWidgets.QComboBox()
        watchlistcombo.addItems([str(i) for i in range(12)])

        watch_button = QtWidgets.QPushButton("  Watch  ")
        watch_button.clicked.connect(self.watch_strategy)
        ...
        label_text = (
            f"{api}|{account}: {full_symbol} - {strategy_name}({class_name})@{engine_id}"
        )
        ...
        hbox.addStretch()
        hbox.addWidget(watchlistcombo)
        hbox.addWidget(watch_button)
    ...
    def watch_strategy(self):
        si = self.watchlistcombo.currentText()
        self.cta_manager.watch_strategy(self._data,si)
