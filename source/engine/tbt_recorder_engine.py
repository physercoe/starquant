...
from ..common.datastruct import (
    ContractData, Event, TickData, BarData, SubscribeRequest, TBTTradeData, TBTEntrustData
)
...
class TBTRecorderEngine(BaseEngine):
    """
    market data recorder 
    """
    config_filename = "config_server.yaml"
    setting_filename = "tbt_data_recorder_setting.json"

# init
    def __init__(self, configfile: str = '', gateway: str = "XTP.MD"):
...
# init functions
    ...
    def load_setting(self):
        ...
        self.tbte_recordings = setting.get("tbte", {})
        self.tbtt_recordings = setting.get("tbtt", {})
        self.tbtbar_recordings = setting.get("tbtbar", {})

    def save_setting(self):
        """"""
        setting = {
            "tbte": self.tbte_recordings,
            "tbtt": self.tbtt_recordings,
            "tbtbar": self.tbtbar_recordings
        }
        save_json(self.setting_filename, setting)

    def register_event(self):
        """"""
        self.event_engine.register(EventType.TBTENTRUST, self.process_tbte_event)
        self.event_engine.register(EventType.TBTTRADE, self.process_tbtt_event)
    ...
    def init_subcribe(self, src: str = 'XTP.MD'):
...
# event handler
    ...
    def process_tbte_event(self, event: Event):
        """"""
        tick = event.data
        self.record_tbte(tick)
    
    def process_tbtt_event(self, event: Event):
        """"""
        tick = event.data
        self.record_tbtt(tick) 
    ...
    def record_tbte(self, tick: TBTEntrustData):
        """"""
        database_manager.save_tbte_data([tick])
    

    def record_tbtt(self, tick: TBTTradeData):
        """"""
        database_manager.save_tbtt_data([tick])
    ...
    def subscribe(self, full_symbol: str, src: str = 'XTP.MD'):
        m = Event(type=EventType.SUBSCRIBE,
        msgtype=MSG_TYPE.MSG_TYPE_SUBSCRIBE_ORDER_TRADE)
        m.destination = src
        m.source = str(self.id)
        req = SubscribeRequest()
        if src == 'XTP.MD':
            req.sym_type = SYMBOL_TYPE.FULL
            req.content = 'SSE T all 0'
        m.data = req
        self._send_sock.send(m.serialize())
    ...
    def clear(self):
        self.tbtbar_recordings.clear()
        self.tbte_recordings.clear()
        self.tbtt_recordings.clear()
        self.save_setting()
        self.put_event()
