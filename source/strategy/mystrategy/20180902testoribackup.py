...
#恢复成本价
#            if context.firedOpenRecoverCost[Context] :

#                if context.buy_qty[Context] > 0 and (tick.last - context.buy_prices[Context]) > context.stoplosstimePoint[Context] * instruments(Context).tick_size() :
#                    context.buyStopLossPoint[Context] = context.buy_prices[Context] 
#                    print('恢复成本价到买价',context.buyStopLossPoint[Context])
#                    context.firedOpenRecoverCost[Context] = False
#                    
#                if context.buy_qty[Context] > 0:
#                    
#                    if context.now - context.BuyTime[Context] <  datetime.timedelta(minutes = context.BuyRecoverTimeLimit[Context]):
#                        context.list_AfterBuy[Context].append(tick.last)
#                    else :
#                        if tick.last - min(context.list_AfterBuy[Context]) > context.BuyRecoverPointLimit[Context]*instruments(Context).tick_size() and min(context.list_AfterBuy[Context]) > context.buyStopLossPoint[Context]:
#                            context.buyStopLossPoint[Context] = min(context.list_AfterBuy[Context])
#                            print('恢复成本价到坑1:',context.buyStopLossPoint[Context],' tick.last:',tick.last)
#                            context.firedOpenRecoverCost[Context] = False 
...
#                            context.H1Pricetmp[Context] = tick.last
#                        else :
#                            if len(context.list_betweenH1_High[Context]) != 0 and min(context.list_betweenH1_High[Context]) > context.buyStopLossPoint[Context] :
#                                context.buyStopLossPoint[Context] = min(context.list_betweenH1_High[Context])
#                                print('恢复成本价到坑2',context.buyStopLossPoint[Context])
#                                context.firedOpenRecoverCost[Context] = False  
...
#                if context.sell_qty[Context] > 0 and (context.sell_prices[Context] - tick.last) > context.stoplosstimePoint[Context] * instruments(Context).tick_size() :
#                    context.sellStopLossPoint[Context] = context.sell_prices[Context]
#                    print('恢复成本价到卖价',context.sellStopLossPoint[Context])
#                    context.firedOpenRecoverCost[Context] = False
                    
                    
#                if context.sell_qty[Context] > 0:
                     
#                    if context.now - context.SellTime[Context] < datetime.timedelta(minutes = context.SellRecoverTimeLimit[Context]):
#                        context.list_AfterSell[Context].append(tick.last)
#                    else :
#                        if tick.last - max(context.list_AfterSell[Context]) > context.SellRecoverPointLimit[Context]*instruments(Context).tick_size() and max(context.list_AfterSell[Context]) < context.sellStopLossPoint[Context]:
#                            context.sellStopLossPoint[Context] = max(context.list_AfterSell[Context])
#                            print('恢复成本价到包1',context.sellStopLossPoint[Context])
#                            context.firedOpenRecoverCost[Context] = False 
...
#                            context.L1Pricetmp[Context] = tick.last
#                        else :
#                            if len(context.list_betweenH1_High[Context]) != 0 and max(context.list_betweenL1_Low[Context]) < context.sellStopLossPoint[Context]:
#                                context.sellStopLossPoint[Context] = max(context.list_betweenL1_Low[Context])
#                                print('恢复成本价到包2',context.sellStopLossPoint[Context])
#                                context.firedOpenRecoverCost[Context] = False 
